...
from llm.planner.models import PlanRequest, PlanResponse, AgentResult, DecideRequest, StepType, TimingMetadata
...
async def execute_plan(request: PlanRequest) -> PlanResponse:
    ...
    logger.info(f"Planner identified {len(decide_responses)} task(s) for session: {request.sid} in {planner_duration:.3f}s")
    ...
    for idx, decide_response in enumerate(decide_responses):
        ...
        if step_type == StepType.EDIT:
            ...
            logger.info(f"EDIT agent completed for step: {step_id} in {agent_duration:.3f}s")
            ...
            logger.info(f"ACT agent completed for step: {step_id} in {agent_duration:.3f}s")
            ...
            logger.info(f"CLARIFY agent completed for step: {step_id} in {agent_duration:.3f}s")
    ...
    logger.info(f"Plan execution completed for session: {request.sid} with {len(results)} result(s) in {overall_duration:.3f}s (planner: {planner_duration:.3f}s, agents: {total_agent_time:.3f}s)")
