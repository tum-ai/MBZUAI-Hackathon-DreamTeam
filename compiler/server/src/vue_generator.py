...
class RenderContext:
    """
    Context object passed to component renderers with helper methods.
    Provides access to VueGenerator methods without tight coupling.
    """
    def __init__(self, vue_generator):
        self.vue_generator = vue_generator
    
    def generate_node(self, node, parent_context="", index_in_parent=None):
        """Delegate to VueGenerator's _generate_node method."""
        return self.vue_generator._generate_node(node, parent_context, index_in_parent)
    
    def resolve_expression(self, expr_obj, is_event_handler=False):
        """Delegate to VueGenerator's _resolve_expression method."""
        return self.vue_generator._resolve_expression(expr_obj, is_event_handler)
    
    def generate_style_string(self, style_obj):
        """Delegate to VueGenerator's _generate_style_string method."""
        return self.vue_generator._generate_style_string(style_obj)
...
class VueGenerator:
    ...
    def _generate_node(self, node, parent_context="", index_in_parent=None):
        ...
        # --- Check if component has a specialized renderer ---
